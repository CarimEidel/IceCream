<%@ page import="models.*, controllers.*, java.util.List, utils.StringUtils" language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
...
<!-- Header -->
   <header id="header">
      <a href="index.jsp"><img class="logo " src="/IceCream/assets/images/index/logo.png"> </a>
      <nav >
        <ul id="navbar">
...
<%
	ProfileModel s=ProfileController.getProfile(product.getSeller_Id());
...
%>
   <section id="store-section" class="store-section-padding2">
      <div class="store-section-container">
        <div class="store-dp" >
          <img src="/IceCream/assets/images/index/IceCream1.jpg" alt="">
        </div>
        
        <div class="store-name" >
          <p class="name"><%= s.getShopName() %>
            <a href="denver.jsp?shopName=<%= s.getShopName() %>"><button class="view-store fa-solid fa-house">View Shop</button></a>
          </p>
        </div>
<%
	UserModel join1=UserController.getJoinedDate(product.getSeller_Id());
	System.out.println(product.getSeller_Id());
%>        
        <div class="store-details">
          <span>Joined <%= join1.getJoinedDate() %> months ago</span>
          
          
<%
	ProductModel prod=ProductController.getCountProduct(product.getSeller_Id());
	System.out.println(product.getSeller_Id());
%>
          <span>Products <%= prod.getProductCount() %></span>
